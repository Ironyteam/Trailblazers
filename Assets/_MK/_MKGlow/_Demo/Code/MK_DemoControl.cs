...
public class MK_DemoControl : MonoBehaviour 
{
	...
	{
		mkGlow.BlurIterations = 7;
		mkGlow.Samples = 3;
		mkGlow.GlowIntensity = 0.6f;
	...
	private void ManageRoom0()
	{
		...
		if(mkGlow.GlowType == MKGlowType.Fullscreen)
		{
			...
			mkGlow.GlowTint = colors[currentRoom0GlowColor];
		}

		GUI.Label(new Rect(1500, 600, 300, 50), "Blur Spread", r2bStyle);
		mkGlow.BlurSpread = GUI.HorizontalSlider(new Rect(1500, 650, 400, 100), mkGlow.BlurSpread, 0.05f, 1.0f);
		GUI.Label(new Rect(1500, 700, 300, 50), "Samples", r2bStyle);
		mkGlow.Samples = Mathf.RoundToInt(GUI.HorizontalSlider(new Rect(1500, 750, 400, 100), mkGlow.Samples, 1, 8));
		GUI.Label(new Rect(1500, 800, 300, 50), "Blur Iterations", r2bStyle);
		mkGlow.BlurIterations = Mathf.RoundToInt(GUI.HorizontalSlider(new Rect(1500, 850, 400, 100), mkGlow.BlurIterations, 0, 10));
		GUI.Label(new Rect(1500, 900, 300, 50), "Glow Intensity", r2bStyle);
		mkGlow.GlowIntensity = GUI.HorizontalSlider(new Rect(1500, 950, 400, 100), mkGlow.GlowIntensity, 0.0f, 1f);
	}
